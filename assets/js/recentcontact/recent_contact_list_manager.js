...
var initRecentContactList = function (cbOK, cbErr) {
    ...
    webim.getRecentContactList(
        ...
        function (resp) {
            ...
            //清空聊天对象列表
            //var sessList =$(".xxim_chatlist").eq(0);
            //sessList.html("");
            if (resp.SessionItem && resp.SessionItem.length > 0) {//如果存在最近会话记录
                ...
                for (var i in resp.SessionItem) {
                    ...
                    if(!tempSess){//先判断是否存在（用于去重），不存在增加一个
                        ...
                        addSess(sessType,webim.Tool.formatText2Html(sessionId), webim.Tool.formatText2Html(sessionNick), sessionImage, item.UnreadMsgCount, 'xxim_chatlist');
                    }
